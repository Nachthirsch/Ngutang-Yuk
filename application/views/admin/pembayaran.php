...
<body>
    <div class="container-fluid">
        <nav class="sidebar">
            ...
            <ul>
                ...
                <li><a href="<?= site_url('admin/pembayaran')?>" class="active"><i class="fas fa-credit-card"></i> Pembayaran</a></li>
        ...
        <main class="main-content">
            ...
            <div class="search-container">
                <input type="text" id="searchInput" placeholder="Cari data pembayaran...">
                    ...
                    <tbody>
                        <?php if(!empty($pembayaran)): ?>
                            <?php foreach($pembayaran as $p): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($p->Pembayaran_ID); ?></td>
                                    <td><?php echo htmlspecialchars($p->Pinjaman_ID); ?></td>
                                    <td><?php echo htmlspecialchars($p->Jumlah); ?></td>
                                    <td><?php echo htmlspecialchars($p->Tanggal_Pembayaran); ?></td>
                                    <td><?php echo htmlspecialchars($p->Bulan_Angsuran); ?></td>
                                    <td><?php echo htmlspecialchars($p->Anggota_ID); ?></td>
                                    <td>
                                        <button class="btn-action btn-primary edit-pembayaran" data-id="<?php echo $p->Pembayaran_ID; ?>"><i class="fas fa-edit"></i></button>
                                        <button class="btn-action btn-danger delete-pembayaran" data-id="<?php echo $p->Pembayaran_ID; ?>"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
    ...
    <!-- Modal -->
    <div class="modal fade" id="editPembayaranModal" tabindex="-1" role="dialog" aria-labelledby="editPembayaranModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editPembayaranForm">
                        ...
                        <div class="form-group">
                            <label for="pinjamanId">Pinjaman ID</label>
                            <input type="text" class="form-control" id="pinjamanId" name="Pinjaman_ID" readonly>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="tanggalPembayaran" name="Tanggal_Pembayaran">
                        </div>
                        <div class="form-group">
                            <label for="bulanAngsuran">Bulan Angsuran</label>
                            <input type="text" class="form-control" id="bulanAngsuran" name="Bulan_Angsuran">
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="savePembayaran">Save changes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        $('.edit-pembayaran').on('click', function() {
            ...
            $.ajax({
                url: '<?= site_url('admin/get_pembayaran_by_id/') ?>' + pembayaranId,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    console.log(response);
                    $('#pembayaranId').val(response.Pembayaran_ID);
                    $('#pinjamanId').val(response.Pinjaman_ID);
                    $('#jumlah').val(response.Jumlah);
                    $('#tanggalPembayaran').val(response.Tanggal_Pembayaran);
                    $('#bulanAngsuran').val(response.Bulan_Angsuran);
        ...
        $('#savePembayaran').on('click', function() {
            var formData = $('#editPembayaranForm').serialize();
            
            $.ajax({
                url: '<?= site_url('admin/update_pembayaran') ?>',
                type: 'POST',
                data: formData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        alert('Gagal memperbarui data pembayaran: ' + response.message);
                    }
                },
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
                    alert('Terjadi kesalahan saat memperbarui data pembayaran');
                }
            });
        });

        $('.delete-pembayaran').on('click', function() {
            var pembayaranId = $(this).data('id');
            if (confirm('Are you sure you want to delete this payment?')) {
                $.ajax({
                    url: '<?= site_url('admin/delete_pembayaran') ?>',
                    type: 'POST',
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            $('button[data-id="' + pembayaranId + '"]').closest('tr').remove();
                        } else {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        alert('Terjadi kesalahan saat menghapus pembayaran: ' + error);
                    }
                });
            }
        });
